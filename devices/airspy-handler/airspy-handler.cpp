...
//
	...
	for (i = 0; i < (int)samplerate_count; i ++) {
	   if (abs (myBuffer [i] - outputRate) < distance) {
	      distance	= abs (myBuffer [i] - outputRate);
	...
	if (selectedRate == 0) {
	   ...
	   fprintf (stderr, "selected samplerate = %d (%d)\n",
...
//	(selectedRate / 1000) vs (outputRate / 1000)
	...
	mapTable_int		= new int16_t [outputRate / 1000];
	mapTable_float		= new float   [outputRate / 1000];
...
//	Here we set the gain and frequency
	...
	if (my_airspy_set_freq (device, frequency) != AIRSPY_SUCCESS)
